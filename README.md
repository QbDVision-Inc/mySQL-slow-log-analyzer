...
This tool helps you determine why a MySQL database is running slowly. Specifically, it converts a MySQL slow query log into a set of CSV files that can be used to figure out why a database is underperforming or running out of memory. 
...
* **Total Lock Time** - The total time MySQL waited while another process had a row locked that it needed.
...
Follow the same instructions for importing the CSV into the same Google Sheet. In addition, select the first 2 columns (A and B) and choose `Insert -> Chart`. You should see a chart like this now:
...
If you find your MySQL DB is running out of memory or rebooting often, or perhaps "Too many connections", this chart will be invaluable. You can see at what time the connection spiked, scroll down to that line and see all of the queries that were running at that time. If you see a steady climb of connections then you're not closing your connections correctly.
...
Usually each thread gets around 17.5 MB of memory (read more on how to calculate all of the memory things [here](https://dba.stackexchange.com/a/256104/24545)).
